...
class TestContentBasedRecommender(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print(f"\n✓ Test data loaded: {len(cls.movies_df)} movies, {len(cls.ratings_df)} ratings")
    ...
    def test_01_initialization(self):
        ...
        print("  ✓ Recommender initialized with correct parameters")
        print(f"  ✓ Weights: genre={self.recommender.genre_weight}, tag={self.recommender.tag_weight}, title={self.recommender.title_weight}")
    ...
    def test_03_genre_extraction(self):
        ...
        print(f"  ✓ Genre features: {n_movies} movies × {n_genres} genres")
        print(f"  ✓ Sparse matrix density: {self.recommender.genre_features.nnz / (n_movies * n_genres):.4f}")
    ...
    def test_04_title_extraction(self):
        ...
        print(f"  ✓ Title features: {n_movies} movies × {n_keywords} keywords")
        print(f"  ✓ Sparse matrix density: {self.recommender.title_features.nnz / (n_movies * n_keywords):.4f}")
    ...
    def test_05_combined_features(self):
        ...
        self.assertTrue(issparse(self.recommender.feature_matrix))
        
        n_movies, n_features = self.recommender.feature_matrix.shape
        self.assertEqual(n_movies, len(self.movies_df))
        
        print(f"  ✓ Combined features: {n_movies} movies × {n_features} total features")
        print(f"  ✓ Memory usage: {self.recommender.feature_matrix.data.nbytes / 1024:.2f} KB")
    ...
    def test_06_similarity_matrix(self):
        ...
        print(f"  ✓ Similarity matrix: {n_movies} × {n_movies}")
        print(f"  ✓ Non-zero entries: {self.recommender.similarity_matrix.nnz}")
        print(f"  ✓ Sparsity: {1 - self.recommender.similarity_matrix.nnz / (n_movies * n_movies):.4f}")
    ...
    def test_07_similarity_threshold(self):
        ...
        print(f"  ✓ Min similarity in matrix: {min_sim:.4f}")
        print(f"  ✓ Threshold: {self.recommender.min_similarity}")
    ...
    def test_08_user_profile_building(self):
        ...
        self.assertTrue(issparse(user_profile))
        
        # Check that profile has same dimension as feature matrix
        self.assertEqual(user_profile.shape[1], self.recommender.feature_matrix.shape[1])
        ...
        print(f"  ✓ User profile created with {user_profile.shape[1]} features")
        print(f"  ✓ Profile norm: {profile_norm:.6f} (normalized)")
    ...
    def test_09_user_profile_weights(self):
        ...
        print(f"  ✓ User 1 ratings: {user_1_ratings['rating'].values}")
        print(f"  ✓ Expected weights: {expected_weights}")
        print(f"  ✓ Weight calculation: (rating - 2.5) / 2.5")
    ...
    def test_10_predict_rating(self):
        ...
        print(f"  ✓ Predicted rating for user 1, movie 4: {predicted_rating:.2f}")
        print(f"  ✓ Rating range valid: 0.5 ≤ {predicted_rating:.2f} ≤ 5.0")
    ...
    def test_11_predict_multiple_users(self):
        ...
        for user_id in [1, 2, 3]:
            ...
            print(f"  ✓ User {user_id}, Movie 5: {pred:.2f}")
    ...
    def test_12_get_recommendations(self):
        ...
        print(f"  ✓ No overlap with user's {len(user_rated)} rated movies")
        if len(recommendations) > 0:
            print(f"  ✓ Top recommendation: {recommendations.iloc[0]['title']} (score: {recommendations.iloc[0]['predicted_rating']:.2f})")
    ...
    def test_13_cold_start_handling(self):
        ...
        print(f"  ✓ Cold-start handled: returned {len(recommendations)} popular movies")
        if len(recommendations) > 0:
            print(f"  ✓ Fallback movie: {recommendations.iloc[0]['title']}")
    ...
    def test_14_similar_items(self):
        ...
        # Get similar movies to movie 1 (Toy Story)
        similar_movies = self.recommender.get_similar_items(movie_id=1, n=2)
        ...
        print(f"  ✓ Found {len(similar_movies)} similar movies to 'Toy Story'")
        for idx, row in similar_movies.iterrows():
            print(f"  ✓ Similar: {row['title']} (similarity: {row.get('similarity', 'N/A')})")
    ...
    def test_15_training_metrics(self):
        ...
        self.assertIsNotNone(self.recommender.training_time)
        self.assertIsNotNone(self.recommender.rmse)
        self.assertIsNotNone(self.recommender.coverage)
        
        self.assertGreater(self.recommender.training_time, 0)
        self.assertGreaterEqual(self.recommender.rmse, 0)
        self.assertGreaterEqual(self.recommender.coverage, 0)
        self.assertLessEqual(self.recommender.coverage, 100)
        
        print(f"  ✓ Training time: {self.recommender.training_time:.2f}s")
        print(f"  ✓ RMSE: {self.recommender.rmse:.4f}")
        print(f"  ✓ Coverage: {self.recommender.coverage:.2f}%")
        print(f"  ✓ Memory usage: {self.recommender.memory_usage_mb:.2f} MB")
    ...
    def test_16_model_save_load(self):
        ...
        self.recommender.save_model(str(test_model_path))
        self.assertTrue(test_model_path.exists(), "Model file should be created")
        
        # Load model
        loaded_recommender = ContentBasedRecommender.load_model(str(test_model_path))
        ...
        print(f"  ✓ Prediction consistency: {original_pred:.4f} = {loaded_pred:.4f}")
    ...
    def test_17_invalid_user(self):
        ...
        print(f"  ✓ Invalid user handled gracefully: returned {result:.2f}")
    ...
    def test_18_invalid_movie(self):
        ...
        print(f"  ✓ Invalid movie handled gracefully: returned {result:.2f}")
    ...
    def test_19_empty_ratings(self):
        ...
        print(f"  ✓ Empty history handled: returned {len(recommendations)} fallback movies")
    ...
    def test_20_prediction_speed(self):
        ...
        print(f"  ✓ Predictions per second: {1000 / avg_time_ms:.0f}")
