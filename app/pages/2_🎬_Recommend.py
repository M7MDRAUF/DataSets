...
with col2:
    ...
    # Check if algorithm is cached/trained
    if selected_algorithm in manager.get_cached_algorithms():
        st.success("✅ Algorithm Ready")
        st.caption("Metrics will appear below after generating recommendations")
    else:
        st.info("⏳ Algorithm Not Loaded")
        st.caption("Will train when you click 'Get Recommendations'")
...
# Only train/generate if button was clicked, but always display if results exist
if get_recs_button:
    # Validate user ID
    ...
    try:
        # Special handling for Hybrid algorithm (long training time)
        if selected_algorithm == AlgorithmType.HYBRID:
            # Use a simple container instead of st.status to avoid UI complexity
            st.info("🚀 Training Hybrid Algorithm... This will take 3-5 minutes. Please be patient!")
            
            # Create a progress placeholder
            progress_placeholder = st.empty()
            progress_placeholder.write("📊 Step 1/5: Initializing...")
            
            try:
                # CRITICAL: Pass suppress_ui=True to prevent nested st.spinner() calls
                progress_placeholder.write("📊 Step 2/5: Training SVD (60s)...")
                
                algorithm = manager.switch_algorithm(selected_algorithm, suppress_ui=True)
                
                progress_placeholder.empty()
                st.success("✅ Hybrid model ready!")
                
            except MemoryError as mem_err:
                progress_placeholder.empty()
                st.error("❌ Out of memory! Try using a smaller dataset or individual algorithms.")
                st.stop()
            except Exception as algo_err:
                progress_placeholder.empty()
                st.error(f"❌ Training failed: {str(algo_err)}")
                import traceback
                st.code(traceback.format_exc())
                st.stop()
        else:
            # Regular algorithms - use standard spinner
            with st.spinner(f"🤖 Loading {selected_algorithm.value} algorithm..."):
                algorithm = manager.switch_algorithm(selected_algorithm)
        
        # Store the trained algorithm in session state for metrics display
        st.session_state.trained_algorithm = algorithm
        st.session_state[f'algo_metrics_{selected_algorithm.value}'] = {
            'rmse': algorithm.metrics.rmse,
            'coverage': algorithm.metrics.coverage,
            'training_time': algorithm.metrics.training_time,
            'memory_usage_mb': algorithm.metrics.memory_usage_mb,
        ...
        # Show LIVE metrics directly from the algorithm object
        st.markdown("### 📊 Algorithm Performance Metrics")
        
        # Debug: Print metrics to terminal
        print(f"🔍 DEBUG Metrics Display:")
        ...
        with metrics_col1:
            st.metric(
                "RMSE", 
                f"{algorithm.metrics.rmse:.4f}" if algorithm.metrics.rmse > 0 else "N/A",
        ...
        with metrics_col2:
            st.metric(
                "Coverage", 
                f"{algorithm.metrics.coverage:.1f}%" if algorithm.metrics.coverage > 0 else "N/A",
                help="% of movies that can be recommended"
            )
        
        with metrics_col3:
            st.metric(
                "Training Time", 
                f"{algorithm.metrics.training_time:.1f}s" if algorithm.metrics.training_time > 0 else "N/A",
                help="Time taken to train/load"
            )
        
        with metrics_col4:
            st.metric(
                "Memory Usage", 
                f"{algorithm.metrics.memory_usage_mb:.1f}MB" if algorithm.metrics.memory_usage_mb > 0 else "N/A",
...
# Display recommendations if they exist in session state (outside button block)
if 'current_recommendations' in st.session_state and st.session_state.current_recommendations is not None:
    try:
        recommendations = st.session_state.current_recommendations
        user_history = st.session_state.get('user_history', pd.DataFrame())
        current_user_id = st.session_state.get('current_user_id', user_id)
        current_algorithm = st.session_state.get('current_algorithm', selected_algorithm)
        
        # Get algorithm object
        if 'trained_algorithm' in st.session_state:
            algorithm = st.session_state.trained_algorithm
        else:
            algorithm = manager.get_algorithm(current_algorithm)
        
        # Main recommendations display
        st.markdown(f"""
        <div class="recommendation-header">
            <h2>🎬 Recommendations for User {current_user_id}</h2>
            <p>Generated by {algorithm.name} • Based on {len(user_history)} ratings</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with metrics_col1:
            st.metric("RMSE", f"{algorithm.metrics.rmse:.4f}" if algorithm.metrics.rmse > 0 else "N/A")
        with metrics_col2:
            st.metric("Coverage", f"{algorithm.metrics.coverage:.1f}%" if algorithm.metrics.coverage > 0 else "N/A")
        with metrics_col3:
            st.metric("Training Time", f"{algorithm.metrics.training_time:.1f}s" if algorithm.metrics.training_time > 0 else "N/A")
        with metrics_col4:
            st.metric("Memory", f"{algorithm.metrics.memory_usage_mb:.1f} MB" if algorithm.metrics.memory_usage_mb > 0 else "N/A")
        ...
        with rec_col:
            ...
            for idx, row in recommendations.iterrows():
                ...
                with col_left:
                    # Explanation button
                    if st.button(f"💡 Why this movie?", key=f"session_explain_{movie_id}_{idx}"):
                        explanation = manager.get_recommendation_explanation(
                            current_algorithm, current_user_id, movie_id
                        )
                        st.markdown(f"""
                        <div class="explanation-box">
                            <strong>Why this recommendation?</strong><br>
                ...
                with col_right:
                    # Feedback buttons in single row
                    if st.button("👍 Like", key=f"session_like_{movie_id}_{idx}"):
                        st.success("Thanks for the feedback! 👍")
                    if st.button("👎 Not interested", key=f"session_dislike_{movie_id}_{idx}"):
                        st.info("Feedback noted! 👎")
        ...
        with profile_col:
            ...
            if len(user_history) > 0:
                ...
                for _, movie_row in user_history.iterrows():
                    movie_info = movies_df[movies_df['movieId'] == movie_row['movieId']]
                    if len(movie_info) > 0:
                        genres_str = movie_info.iloc[0]['genres'].split('|')
                        for genre in genres_str:
                            genre_counts[genre] = genre_counts.get(genre, 0) + 1
                
                # Show top 5 genres
                top_genres = sorted(genre_counts.items(), key=lambda x: x[1], reverse=True)[:5]
                total_ratings_count = len(user_history)
                
                for genre, count in top_genres:
                    percentage = (count / total_ratings_count) * 100
                    emoji = get_genre_emoji(genre)
                    st.markdown(f"{emoji} **{genre}**: {percentage:.1f}%")
        ...
        if current_algorithm != AlgorithmType.HYBRID:
            st.markdown("---")
            st.info(f"💡 **Try the Hybrid algorithm** for potentially better recommendations!")
            
    except Exception as display_err:
        st.error(f"Error displaying recommendations: {display_err}")
